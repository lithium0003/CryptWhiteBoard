...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    lazy var persistentContainer: NSPersistentContainer = {
        ...
        */
        let container = NSPersistentContainer(name: "CryptWhiteboard")
...
extension AppDelegate: UNUserNotificationCenterDelegate {
    func userNotificationCenter(_ center: UNUserNotificationCenter, willPresent notification: UNNotification, withCompletionHandler completionHandler: @escaping (UNNotificationPresentationOptions) -> Void) {
        print("notify\(callCount)")
        OSAtomicIncrement64Barrier(&callCount)
        DispatchQueue.main.asyncAfter(deadline: .now()+2) {
            if OSAtomicDecrement64Barrier(&self.callCount) == 0 {
                self.drawRequest()
            }
    ...
    func drawRequest() {
        ...
        guard let topvc = vc.presentedViewController as? ViewController else {
            return
        }
        topvc.board.readStroke()
        topvc.remote.resetBadge()
    }
