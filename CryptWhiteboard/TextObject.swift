...
class TextObject: DrawObject {
    ...
    override class func create(recordId: String?, data: Data, cmd: String, container: DrawObjectContaier) -> DrawObject? {
        return TextObject(recordId: recordId, data: data, cmd: cmd)
    }
    ...
    override init?(recordId: String?, data: Data, cmd: String) {
        ...
        let fonts = nextData.withUnsafeBytes { pointer in
            pointer.load(as: Double.self)
        }
        nextData = nextData.advanced(by: MemoryLayout<Double>.size)
        let fontc = nextData.withUnsafeBytes { pointer in
            pointer.load(as: Int.self)
        }
        nextData = nextData.advanced(by: MemoryLayout<Int>.size)
        let fontData = nextData.subdata(in: 0..<fontc)
        nextData = nextData.advanced(by: fontc)
        if let fontDescriptor = try? NSKeyedUnarchiver.unarchivedObject(ofClass: UIFontDescriptor.self, from: fontData) {
            font = UIFont(descriptor: fontDescriptor, size: CGFloat(fonts))
        }
        else {
            font = .systemFont(ofSize: CGFloat(fonts))
        }
        
        str = String(bytes: nextData, encoding: .utf8) ?? ""
    }
    ...
    override func save(writer: (String, Data, @escaping (Bool?) -> Void) -> String?, finish: ((Bool?) -> Void)? = nil) -> String? {
        ...
        let cmd = String(format: "text;R=%f;G=%f;B=%f;A=%f", Red, Green, Blue, Alpha)
    ...
    override func getLocationData() -> Data {
        ...
        guard let fontd = try? NSKeyedArchiver.archivedData(withRootObject: font.fontDescriptor, requiringSecureCoding: true) else {
            return Data()
        }
        var fontc = fontd.count
        var fonts = Double(font.pointSize)
        data.append(Data(bytes: &fonts, count: MemoryLayout<Double>.size))
        data.append(Data(bytes: &fontc, count: MemoryLayout<Int>.size))
        data.append(fontd)
    ...
    override func drawInContext(_ context: CGContext) {
        ...
        NSAttributedString(string: str, attributes: [.font: font, .foregroundColor: textColor]).draw(in: targetRect)
